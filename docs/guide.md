...
* __Un projet mutlijoueur est quelque chose de long à mettre en place. Il faut l'anticiper pour ne pas prendre trop de retard.__
...
* __Votre projet doit implémenter un mode multi-joueur, faite-le en priorité !.__ Le plus grosse erreur est de réaliser ce mode multi-joueurs au milieu du developpement de votre projet. Aisni, vous vous retrouverez avec un code scinder en deux (une partie multi et une partie solo) ce qui va vite devenir compliquer à gérer. La bibliothèque [Photon](https://www.photonengine.com/en-us/Photon) est sans doute la plus simple à utiliser. Eviter l'utilisation de Mirror souvent plus complexe, plus supportés et avec une documentation pas forcément à jour.

* __Le mode à un seul joueur doit être un serveur local privé à un joueur.__
Utiliser des Sockets directement est du suicide. L'utilisation de Photon [Photon](https://www.photonengine.com/en-us/Photon) est ainsi à privilégié car gratuite et possède une documentation suffisament complète.
...
* __Concentrez vous sur la version Windows.__ Unity est un moteur de jeu multiplateforme, mais rien de vous empêche d’adapter votre jeu sur d’autres supports. Une version Windows pleinement fonctionnelle vaut plus que des versions Windows, Mac et Linux boîteuses. Le projet final étant tester sur Windows (et à présenter sur cette même platforme), la portabilité sur d'autre systèmes n'est qu'une amélioration non-essentielles.
